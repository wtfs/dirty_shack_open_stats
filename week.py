...
print("Generate Week Plot data")
sum_open_plot=np.linspace(0,0,num=60*60*24*7)
sum_open_plot_x=np.linspace(60*60*24*7,0,num=60*60*24*7)
sum_open_plot_label=[]

print("Generate Week Plot labels")
...
for i in range(0,to_range_limit-2):
    if (open_time[i+1]-open_time[i]) < min_close_time and x_open_start == None:
        ...
        set_range(x_open_start, x_open_stop, 1, (60*60*24*7))
        x_open_start=None
...
for i in range(0,7):
    plt.annotate("test",xy=(i*(24*60*60),240))
